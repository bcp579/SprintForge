...
export default function ActiveSprint() {
  ...
  const handleLogHours = async (taskId, itemId) => { /* Same as before */
    const hours = logHoursInput[taskId];
    if (!hours || hours <= 0) return alert("Please enter valid hours.");
    await fetch('/api/tasks', { method: 'PUT', body: JSON.stringify({ taskId, hours, backlogItemId: itemId }) });
  ...
  const totalRemaining = sprint.items.reduce((sum, item) => sum + item.remainingEffort, 0);
  
  const burndownData = [
    { day: 'Start', ideal: totalOriginal, actual: totalOriginal },
    { day: 'Mid-Sprint', ideal: totalOriginal / 2, actual: totalOriginal - ((totalOriginal - totalRemaining) / 2) },
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 mb-8 flex justify-between items-center border-l-8 border-l-blue-600">
        ...
        <div className="text-right flex flex-col items-end gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500">Remaining Effort: <span className="font-bold text-blue-600">{totalRemaining} hrs</span></p>
          </div>
          <button 
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {sprint.items.map(item => (
            <div key={item.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative">
              ...
              {/* If item is done, show a green overlay! */}
              ...
              <div className={`p-4 border-b flex justify-between items-center ${item.remainingEffort === 0 ? 'bg-green-50' : 'bg-slate-50'}`}>
                <div>
                  <h3 className={`font-bold text-lg ${item.remainingEffort === 0 ? 'text-green-800 line-through' : 'text-slate-800'}`}>{item.description}</h3>
                ...
                <div className="text-right flex items-center gap-4">
                  ...
                  <div className={`text-center px-3 py-1 rounded border ${item.remainingEffort === 0 ? 'bg-green-100 border-green-200' : 'bg-blue-50 border-blue-100'}`}>
                    <p className={`text-[10px] uppercase font-bold ${item.remainingEffort === 0 ? 'text-green-600' : 'text-blue-500'}`}>Remaining</p>
                    <p className={`font-mono font-bold text-lg ${item.remainingEffort === 0 ? 'text-green-700' : 'text-blue-700'}`}>{item.remainingEffort}h</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4">
                ...
                {/* Add Task Input (Hide if item is completed) */}
                {item.remainingEffort > 0 && (
          ...
          <div className="bg-slate-900 rounded-xl shadow-md border border-slate-800 p-5 sticky top-6 text-white">
            ...
            <div className="h-64 w-full bg-slate-800 rounded-lg p-2 mb-6">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={burndownData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Line type="monotone" dataKey="actual" name="Remaining Effort" stroke="#3b82f6" strokeWidth={3} activeDot={{ r: 8 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between border-b border-slate-700 pb-2">
                ...
                <span className="font-mono font-bold text-green-400">{totalOriginal - totalRemaining} hrs</span>
              </div>
